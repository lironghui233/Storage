 ...
 * @version: 
 * @Author: Darren
 * @Date: 2019-10-19 11:29:34
 * @LastEditors: Darren
 * @LastEditTime: 2021-04-19 15:08:32
 */
...
int DeflatingStream::Deflate(char *inBuf, size_t inLen, std::string &outStr)
{
    ...
    if (!_eof)
    {
        for (;;)
        {
            ...
            {
                outStr.append(_buffer, _buffer + DEFLATE_BUFFER_SIZE);      // 要把数据拷贝走，输出缓冲区已经没有了
                _zstr.next_out = (unsigned char *)_buffer;  // 重置输出缓冲区
