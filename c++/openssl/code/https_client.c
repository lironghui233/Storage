...
static int https_parser_url(const char* url,char **host,int *port,char **path)
{
    ...
    int host_len = host_port-url-i;  
    int path_len = strlen(temp);
    char *host_temp = (char *)malloc(host_len + 1);  
    if(host_temp == NULL)
    {
    ...
    char *path_temp = (char *)malloc(path_len + 1); 
    if(path_temp == NULL)
    {
...
int main()
{
    ...
    https_init(&https_ct,"https://www.lsking.com/");
